...
Two jobs: (1) judge whether the item is **user-facing** and catch **technical work** that isn't
flagged as an enabler; (2) check the **sources** needed to understand it, and flag **conflicts**.
**Cite** evidence. **Read-only — inspect only; never edit, create, or run anything.**
...
2. **Sources** — are the item's **linked sources** enough to understand it? Name any **additional source needed** that isn't linked (a missing **load-bearing** source is blocking; nice-to-have is recorded).
3. **Conflicts** — does the input **contradict itself** or a known source? Raise it; don't resolve silently.
